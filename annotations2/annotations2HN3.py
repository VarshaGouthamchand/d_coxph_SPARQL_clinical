...
endpoint = "http://localhost:7204/repositories/userRepo/statements"
...
INSERT  
    ...
    GRAPH <http://annotation2.local/>
    {
     ...
     db:data.dataset rdf:type owl:Class.
    
     db:data.dataset dbo:table db:data.
    
     db:data.dataset rdfs:label "Dataset".
     
     db:data.dataset owl:equivalentClass ncit:C25474.
    
     ?tablerow dbo:has_column ?dataset.

     ?dataset dbo:has_cell ?datavalue.

     ?datavalue dbo:has_value "HN3".
    
     ?dataset rdf:type db:data.dataset.  
...
addMapping("N3a", "http://ncicb.nci.nih.gov/xml/owl/EVS/Thesaurus.owl#C48786",
           "http://ncicb.nci.nih.gov/xml/owl/EVS/Thesaurus.owl#C48884")
