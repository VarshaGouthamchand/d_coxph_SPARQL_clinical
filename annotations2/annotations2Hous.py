...
endpoint = "http://localhost:7202/repositories/userRepo/statements"
...
INSERT  
    ...
    GRAPH <http://annotation2.local/>
    {
     ...
     db:hnscc_clinical_data.dataset rdf:type owl:Class.
    
     db:hnscc_clinical_data.dataset dbo:table db:hnscc_clinical_data.
    
     db:hnscc_clinical_data.dataset rdfs:label "Dataset".
     
     db:hnscc_clinical_data.dataset owl:equivalentClass ncit:C25474.
    
     ?tablerow dbo:has_column ?dataset.

     ?dataset dbo:has_cell ?datavalue.

     ?datavalue dbo:has_value "Houston".
    
     ?dataset rdf:type db:hnscc_clinical_data.dataset.  
...
where 
{
    ...
    ?tablerow rdf:type db:hnscc_clinical_data.
